    ...
    <div className="shadow-xl p-10 my-5 rounded-lg hover:shadow-2xl grid grid-cols-9 justify-between bg-white">
      ...
      <div
        ...
        onClick={() =>
          navigate("/seeker-single-job-view", {
            state: { data: job.job_id.id },
          })
        }
      >
        <h1 className="capitalize text-xl font-bold pt-3">{job.job_id.job_title} </h1>
        <div className="flex pt-3">
          <p className="mr-5 text-gray-600">
            <i class="fa-solid fa-user-tie"></i> &nbsp;
            {job.job_id.category?.category_name}{" "}
          </p>
          <p className="text-gray-600">
            <i class="fa-thin fa-briefcase"></i> &nbsp; {job.job_id.salary_range}{" "}
          </p>
        </div>
        <div className="py-3">
          <p className="italic text-gray-400">{job.job_id.short_description}</p>
        </div>
      </div>
      <div className="col-span-2 flex flex-col items-center">
        ...
        <button
          ...
          onClick={() =>
            RemoveFromFavourite(job.job_id.id)
          }
        >
          Remove
        </button>
        <p className="text-center pt-3 italic text-gray-600">
          Last Date : {job.job_id.last_date}{" "}
        </p>
      </div>
    </div>
  );
